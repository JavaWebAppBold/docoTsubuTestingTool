...
public class WebRequestBuilder {
	...
	public static HttpRequest post(URI uri, WebQuery params) {
		return HttpRequest.newBuilder(uri)
			.header(CONTENT_TYPE_KEY, CONTENT_TYPE_VALUE)
			// .POST(HttpRequest.BodyPublishers.ofString(params.toQuery()))
			.POST(HttpRequest.BodyPublishers.ofString("name=hoge&pass=1234"))
			.build();
	}
	...
	public static HttpRequest post(URI uri, String params) {
		return HttpRequest.newBuilder(uri)
			...
			.POST(HttpRequest.BodyPublishers.ofString(URLEncoder.encode("name=hoge&pass=1234)", StandardCharsets.UTF_8)))
			// .POST(HttpRequest.BodyPublishers.ofString("pass=1234"))
			.build();
	}
